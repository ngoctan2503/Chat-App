...
public class Service {
    ...
    Model_ChatHistoryRequest request = new Model_ChatHistoryRequest(fromUserID, toUserID);
    client.emit("chat_history", request, new Ack() {
        @Override
        public void call(Object... os) {
            if (os.length > 0) {
                List<Model_Receive_Message> chatHistory = new ArrayList<>();
                for (Object o : os) {
                    chatHistory.add(new Model_Receive_Message(o));
                }
                PublicEvent.getInstance().getEventChat().loadChatHistory(chatHistory);
            }
        }
    });
}
